import { TopContainer, Title, SectionTitle, Paragraph, LinkToTab } from '@/components/page-components';
...
We conclude by describing a setup where users can trustlessly sell execution of transaction themselves on Ethereum, laying out a potential construction of stealth transactions. Interestingly, this setup could be generalized to any kind of contract interactions - aka *stealth interactions*.
...
<div className="flex pl-10 justify-start"> 
    ...
    <li>
        Alice has this valuable piece of information, consisting of knowing an odd number. She asks 1 ether for it.
    </li>
    ...
    <li>
        Alice can now sell to Bob her odd number. She will compute a single proof consisting of:
        <ul className="pl-5 list-disc list-outside">
            <li>
            ...
            <li>
                Showing that she used the same shared key as Bob expected and that the odd number being sold is the same as the one committed earlier - the commitment proof.
            </li>
        </ul>
    </li>
    </ol>
...
<br />
Placing an ask on an Ethereum address is not very different from placing an ask on any other kind of asset. 
...
<div className="flex pl-10 justify-start"> 
    <ol className="list-decimal">
        <li>
            A seller commits to an address for which he would be ready to sell the private key and posts a corresponding price. 
        </li>
        <li>
            A buyer escrows the corresponding price amount in ETH on a smart contract (the "market") and posts his public key . He computes:
            <ul className="pl-5 list-disc"> 
                <li>
        ...
        <li>
            ...
            <ul className="pl-5 list-disc"> 
                ...
                <li>
                Encryption correctness: the private key has been correctly poseidon encrypted
                </li>
                <li>
...
<div className="flex justify-center">
    <Image alt="ask-eth" style={{maxWidth: "100%"}} src={askPrivateKeySetup} />
</div>

<br />

We provide a circuit for this setup <a className="underline" target={"_blank"} href={"https://github.com/dmpierre/eth-private-market/blob/main/packages/circuits/circom/eth-address/no-ecdh-check/sellETHAddressNoECDH.circom"}> here</a>.
...
<div className="flex pl-10 justify-start"> 
    <ol className="list-decimal">
        <li>
            A buyer places a bid on an Ethereum address and escrows a corresponding amount of ETH on the market contract. 
        </li>
        <li>
            A seller posts on-chain a proof: 
            <ul className="pl-5 list-disc"> 
                <li>Assertion correctness: the Ethereum address can be derived from the provided private key. </li>
                <li>Encryption correctness: the private key has been correctly encrypted  </li>
                <li>Commitments correctness: the seller used the buyer's committed public key point. The derived address corresponds to the address being bid. </li>
                <li>Key exchange correctness: the seller's communicated public key point corresponds to the one that has been used to compute the shared encryption key. </li>
            </ul>
        </li>
    </ol>
</div>
...
<div className="flex justify-center">
    <Image alt="ask-eth" style={{maxWidth: "100%"}} src={bidPrivateKeySetup} />
</div>
...
We provide a circuit for this setup <a className="underline" target={"_blank"} href={"https://github.com/dmpierre/eth-private-market/blob/main/packages/circuits/circom/eth-address/ecdh-check/sellETHAddressECDH.circom"}> here</a>.
...
<div className="flex pl-10 justify-start"> 
    <ol className="list-decimal">
        <li>
        A seller commits to a public keypair that he claims to be able to sign messages with and posts a price.
        </li>
        <li>
        Offchain, a buyer orders the signature by escrowing the corresponding price amount in ETH on the market and posts his public key point. He computes:
            <ul className="pl-5 list-disc"> 
                <li>an ECDH value (the "shared key") using the seller's public key point.</li>
                <li>a commitment to the shared key.</li>
                <li>a message to be signed. Here, it consists of the buyer's public key hash.</li>
            </ul>
        </li>
        <li>
        The seller posts on-chain a proof showing:
            <ul className="pl-5 list-disc"> 
                <li>assertion correctness: the signature has been correctly signed over the required hash.</li>
                <li>encryption correctness: the signature has been encrypted correctly.</li>
                <li>commitments correctness: the encryption key corresponds to the committed shared key. The signature's message corresponds to the buyer's on-chain committed hash.</li>
            </ul>
        </li>
    </ol>
</div>
...
We provide a corresponding circuit for this setup <a className="underline" rel={'noreferrer'} target={"_blank"} href="https://github.com/dmpierre/eth-private-market/blob/main/packages/circuits/circom/eddsa-signature/sellSigPublicMessageEdDSA.circom">here</a>.
...
The groth16 proof that we will place an ask for is the same kind of proof that is used on apps such as <a rel={"noreferrer"} target={"_blank"} href="https://twitter.com/heyanonxyz?lang=en">heyanon </a>.
...
<div className="flex pl-10 justify-start"> 
    <ol className="list-decimal">
        <li>
        A seller commits to a group root $r$ to which he claims to have access to and a verification key hash $v$, associated with the verification key used in the proof-gated service.
        </li>
        <li>
        Offchain, a buyer computes a shared key using the seller's public key point. He then orders a proof by escrowing the corresponding price amount in ETH on the market, posting his public key point and committing to a message he wishes to be posted within the group. 
        </li>
        <li>
        The seller posts on-chain a proof showing:
            <ul className="pl-5 list-disc"> 
                <li>assertion correctness: the groth16 proof being sold is correct.</li>
                <li>encryption correctness: the proof has been correctly encrypted.</li>
                <li>commitments correctness: hashes of the encryption and verification keys are correct. The sold proof's group root and message hash correspond to the initially committed values.</li>
            </ul>
        </li>
    </ol>
</div>
...
<br />
We provide a corresponding circuit for this setup <LinkToTab href="https://github.com/dmpierre/eth-private-market/blob/main/packages/circuits/circom/proof-sig-merkle/verifyAndEncryptSigMerkleProof.circom" text="here" />.
...
<div className="flex pl-10 justify-start"> 
    <ol className="list-decimal">
        <li>
        Alice signals her intent to execute a transaction by placing an ask. The transaction will be executed provided some ETH amount is placed in escrow on the market.
        </li>
        <li>
        Bob wants Alice to send some ETH to an address he holds the private key of. He escrows some ETH on the market for Alice. He computes:
            <ul className="pl-5 list-disc"> 
                <li>a shared key using Alice's public key point</li>
                <li>a commitment to the shared key</li>
                <li>the encrypted transaction data he wants Alice to execute. It consists of an address and an amount to be transferred.</li>
                <li>a commitment to the transaction data. It could consist of $h(address, amount)$.</li>
            </ul>
        </li>
        <li>
        Alice decrypts the transaction data. She sends to the specified address the corresponding amount. She will get the escrow by providing a proof consisting of:
            <ul className="pl-5 list-disc"> 
                <li>Assertion correctness: using a merkle path resolving to a transaction trie root, Alice shows that there is a transaction receipt within the transaction receipt trie that satifies what Bob initially requested as transaction parameters. </li>
                <li>Encryption correctness: Alice encrypted the transaction receipt using the shared key. </li>
                <li>Commitment correctness: The hash of the executed transaction is equal to the commitment that has been posted by Bob. The transaction receipt has been encrypted using the shared key.</li>
            </ul>
        </li>
    </ol>
</div>
...
<SectionTitle text={'Conclusion'} size={'text-xl'} />

<br />

This work would not have been possible without all the help I received from the 0xPARC team. The Vietnam residency was fire.

<br />

Also, this work is heavily indebted with the seminal work on recursivity that was done by Nalin.
